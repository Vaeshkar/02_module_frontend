...
function listNamesAndEmails(users) {
 ...
 /*  const validUsers = users.filter((user) => {
  ...
  // Filter to search for username and email.
  // Map to return a key/value array
  try {
    return users.filter(user => user.username && user.email).map(user => [user.username, user.email])
  } catch (e) {
    console.error(e)
  }
}
