...
function decrementOrRemoveFromShoppingCart(cart, itemId) {
  ...
  const foundItem = cart.find(item => item.itemId === itemId);
  if (!foundItem) {
    throw new Error("Item not found.");
  }

  // Step 2: Validate item structure
  if (!("count" in foundItem) || !Number.isFinite(foundItem.count) || foundItem.count <= 0) {
    throw new Error("Invalid item.");
  }

  // Step 3: Create a new cart with modifications
  const updatedCart = [];
  ...
  for (const item of cart) {
    if (item.itemId === itemId) {
      ...
      if (newCount > 0) {
        updatedCart.push({ ...item, count: newCount });
      }
      // If newCount is 0, don't push it — it's removed
    } else {
      updatedCart.push(item); // Other items stays
    }
  }
